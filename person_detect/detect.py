...
#        print(end - start)
    ...
    for x in output:
        ...
        det_names.append("{}/det_{}_{}".format(args.det, i ,imlist[0].split("/")[-1]))
        i += 1    
    ''' 
    orig_ims = list(map(lambda x: write(x, im_batches, orig_ims), output))
    det_names = pd.Series(imlist).apply(lambda x: "{}/det_{}".format(args.det,x.split("/")[-1]))
    
    #det_names = pd.Series(det_names)
        
    
    list(map(cv2.imwrite, det_names, orig_ims))
    # list(map(cv2.imwrite, det_names, output_imgs))
